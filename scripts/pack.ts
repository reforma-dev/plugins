...
const VENDOR_PLUGIN_ROOTS = [
    '${CLAUDE_PLUGIN_ROOT}',
    '${CODEX_PLUGIN_ROOT}',
    '${CURSOR_PLUGIN_ROOT}',
] as const;
const REFORMA_PLUGIN_DIR_PLACEHOLDER = '${REFORMA_PLUGIN_DIR}';
...
const CANONICAL_HOOKS_PATH = './hooks/hooks.json';
...
type HookOffer = { name: string; description?: string };
...
function readHooksJson(file: string): Record<string, unknown> | undefined {
    if (!existsSync(file)) {
        return undefined;
    }
...
function normalizeHooksConfig(raw: Record<string, unknown>): Record<string, unknown> {
...
function buildHookOffers(config: Record<string, unknown>): HookOffer[] | undefined {
    ...
    for (const [event, groups] of Object.entries(bag)) {
        ...
        offers.push({
            name: event,
            description: `${ handlerCount } handler(s)${ matcherNote }`,
        });
    }
...
function normalizePluginHooks(pluginDir: string): void {
    const manifestPath = findManifestPath(pluginDir);
    ...
    writeFileSync(join(hooksDir, 'hooks.json'), `${ JSON.stringify(canonical, null, 4) }\n`);
