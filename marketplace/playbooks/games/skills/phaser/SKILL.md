...
4. **Client island** — Phaser runs in a `'use client'` component with `dynamic(..., { ssr: false })`. Next already bundles.
...
This app already exists. Add Phaser and mount a canvas:
...
- Score HUD is optional — keep it out of `SAFE_ZONE.TOP` if the page already has chrome
- Use parallel scenes for UI overlays (pause menu) only when requested

### HUD band

Keep UI below `SAFE_ZONE.TOP` (about 8% of `GAME.HEIGHT`) so page chrome does not cover buttons.
