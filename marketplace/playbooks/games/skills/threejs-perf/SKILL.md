...
Performance patterns for Three.js games, backed by measured before/after numbers on Three.js r183 (headless Chromium, Apple M1 Pro, software WebGL).
...
Headless Chromium 147, Three.js r183, Apple M1 Pro, 30 warmup + 180 sample frames, median of 3 runs.
...
- **FPS and frame-time p95 are unreliable in headless Chromium.** Software WebGL (SwiftShader) bottlenecks on fragment shading of ~90 MB of visible geometry regardless of draw-call count. On real hardware WebGL, the FPS gap would be substantially larger — baseline would drop to single-digit FPS under real fill, and optimized would hit vsync cleanly.
