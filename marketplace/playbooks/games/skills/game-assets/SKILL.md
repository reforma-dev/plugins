...
- Read Constants under `app/play/game/` for entity types, colors, sizes
...
1. Create `app/play/game/PixelRenderer.ts` with `renderPixelArt()` and `renderSpriteSheet()`
...
- [ ] `PixelRenderer.ts` created under `app/play/game/`
