...
```js
// On score change — floating "+1" text near the action
eventBus.on(Events.SCORE_CHANGED, ({ score }) => {
  const floater = scene.add
    .text(playerX + 30, playerY - 20, "+1", {
      fontSize: "20px",
...
const curtain = scene.add
  .rectangle(0, GAME_CONFIG.height / 2, 0, GAME_CONFIG.height, 0x000000)
  .setOrigin(0, 0.5)
  .setDepth(1000);
scene.tweens.add({
  targets: curtain,
