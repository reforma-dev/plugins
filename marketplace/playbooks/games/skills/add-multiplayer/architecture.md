...
How the multiplayer layer fits into the game-creator architecture (EventBus, GameState, Constants, single-orchestrator).
...
All events use the existing `domain:action` form. Append these constants to EventBus under a `// === Multiplayer ===` banner.
...
`remotePlayers` is owned by `RemotePlayerRegistry`; GameState is the canonical home so screenshots and code reads see one object.
...
Both are valid. Read Constants first to decide which pattern to use, then mirror it in NetworkManager's import. Don't force one onto the other.
...
The dev fallback in `createGame` reads `process.env.NEXT_PUBLIC_MULTIPLAYER_SERVER_URL` first, then falls back to `Constants.MULTIPLAYER.SERVER_URL`. This lets local preview point at `http://127.0.0.1:1999` without committing dev URLs to source.
