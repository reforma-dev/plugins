...
Keep this file in sync with the client's wire types (mirror under `app/play/game/multiplayer/` if you want).
...
First run prompts a Cloudflare login (browser-based OAuth). The output prints the deployed URL — capture it for `Constants.MULTIPLAYER.SERVER_URL`.
