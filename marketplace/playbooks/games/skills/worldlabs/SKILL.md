...
**Tested & working** with SparkJS 2.0 on Three.js r180.

### Install SparkJS 2.0

```bash
bun add @sparkjsdev/spark@^2.0.0
```
...
**Three.js peer version**: Spark 2.0 declares `three@^0.180.0` as its peer dependency. Three treats every 0.x minor as potentially breaking, so `three@^0.181` or newer can fail the install. Pin `three` to `^0.180.0` in the app `package.json` — same pin as the `threejs-3d` sketch.
  ...
  if (WORLD.colliderPath) {
    promises.push(
      (async () => {
        ...
        _colliderMesh.position.set(
          WORLD.position.x,
          WORLD.position.y,
          WORLD.position.z,
        );
        _colliderMesh.traverse((c) => {
...
## What a complete island includes

- World Labs Gaussian Splat environment
- Animated character (walk/run/idle)
- OrbitControls third-person camera
- Collider-mesh ground raycasting
- Panorama skybox

Wire under `app/play/game/`. Preview this Next app. **PageScreenshot** `/play`.
...
**Fix:** Pin to 2.0 or later: `bun add @sparkjsdev/spark@^2.0.0`. Check `node_modules/@sparkjsdev/spark/package.json` to confirm.
