...
>
> **HUD band:** keep UI below `SAFE_ZONE.TOP` (~8% of `GAME.HEIGHT`).
>
> After wiring, the orchestrator **PageScreenshot**s `/play`.
>
...
- Read EventBus, Constants, GameState under `app/play/game/` (or the named route)
- List entity files next to them
...
- **Entities**: [list entity names from `app/play/game/`]
...
>
> - `app/play/game/` — Constants, EventBus, GameState, scenes
...
>
> 1. Read entity files under `app/play/game/` to find `generateTexture()` / `fillCircle()` calls
> 2. Choose the palette that matches the game's theme (DARK, BRIGHT, or RETRO)
> 3. Create `app/play/game/PixelRenderer.ts` — the `renderPixelArt()` + `renderSpriteSheet()` utilities
> 4. Create `app/play/game/sprites/palette.ts` with the chosen palette
> 5. Create sprite data files (`player.ts`, `enemies.ts`, `items.ts`, `projectiles.ts`) with pixel matrices
> 6. Create `app/play/game/sprites/tiles.ts` with background tiles (ground variants, decorative elements)
...
>
> - Install SparkJS: `bun add @sparkjsdev/spark`
...
>
> 1. Create `app/play/game/AssetLoader.ts` — **CRITICAL: use `SkeletonUtils.clone()` for rigged models** (regular `.clone()` breaks skeleton bindings -> T-pose). Import from `three/addons/utils/SkeletonUtils.js`.
...
> - `app/play/game/sprites/` — all sprite data, palettes, and background tiles
...
>
> - `app/play/game/AssetLoader.ts` — model loader with SkeletonUtils
...
>
> **2. Every-Action Effects (wire to SPECTACLE\_\* events from Step 1):**
>
...
> 2. Create `app/play/game/audio/AudioManager.ts` — AudioContext init, master gain, BGM sequencer play/stop
> 3. Create `app/play/game/audio/music.ts` — BGM patterns as note arrays using the Web Audio step sequencer
> 4. Create `app/play/game/audio/sfx.ts` — SFX using Web Audio API (OscillatorNode + GainNode + BiquadFilterNode)
> 5. Create `app/play/game/audio/AudioBridge.ts` — wire EventBus events to audio
...
>
> **Next up:** preview the island and screenshot `/play`. Ready?
