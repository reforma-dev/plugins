...
- Read Constants under `app/play/game/` (or the named route) for the current color palette and config values
...
- **PageScreenshot** `/play` (or the named route) for each scene. Judge colors, spacing, and atmosphere from the shot, not from code alone.
...
| **Safe Zone**                       | Are all UI elements below `SAFE_ZONE.TOP`? Nothing clipped by page chrome?                                                                                                                                                                                                                                                          |
...
eventBus.on(Events.SPECTACLE_COMBO, ({ combo }) => {
  ...
  const text = scene.add
    .text(GAME.WIDTH / 2, GAME.HEIGHT * 0.3, `${combo}x`, {
      fontSize: `${size}px`,
...
eventBus.on(Events.SPECTACLE_STREAK, ({ streak }) => {
  ...
  const text = scene.add
    .text(GAME.WIDTH / 2, GAME.HEIGHT / 2, label, {
      fontSize: "80px",
      fontFamily: "Arial Black",
      color: "#ffffff",
      stroke: "#000000",
...
1. Gameplay — background, entities, atmosphere
2. After fail — game-over polish and score
3. After restart — transitions still clean

Judge color cohesion, visual hierarchy, and atmosphere from the shots.
...
6. **PageScreenshot** before/after if the change is visual
