...
## File: `app/play/game/multiplayer/MultiplayerClient.ts`
...
```js
// app/play/game/multiplayer/MultiplayerClient.ts
...
## File: `app/play/game/multiplayer/RemotePlayerRegistry.ts`
...
```js
// app/play/game/multiplayer/RemotePlayerRegistry.ts
// Tracks remote players keyed by playerId. Mirrors entries into gameState.multiplayer.remotePlayers
// so GameState can serialize remote players with one read. Provides stale-eviction.
...
export class RemotePlayerRegistry {
  ...
  list() {
    return Object.entries(this.gameState.multiplayer.remotePlayers).map(
      ([id, p]) => ({ id, ...p }),
    );
  }
...
## File: `app/play/game/systems/NetworkManager.ts`
...
```js
// app/play/game/systems/NetworkManager.ts
...
export class NetworkManager {
  ...
  _wireClient() {
    ...
    this.client.onMessage((msg) => {
      switch (msg?.type) {
        case "welcome":
          return this._onWelcome(msg);
        case "player-joined":
          return this._onPlayerJoined(msg);
        case "player-left":
          return this._onPlayerLeft(msg);
        case "state":
          return this._onState(msg);
        case "turn":
          return this._onTurn(msg);
        case "reject":
          console.warn("[NetworkManager] server rejected", msg.reason);
          return;
        default:
          return;
      }
  ...
  _wireGameEvents() {
    ...
    this.eventBus.on(
      Events.MULTIPLAYER_LEAVE_ROOM,
      this.boundHandlers.leaveRoom,
    );
  ...
  _unwireGameEvents() {
    this.eventBus.off(
      Events.MULTIPLAYER_JOIN_ROOM,
      this.boundHandlers.joinRoom,
    );
    this.eventBus.off(
      Events.MULTIPLAYER_LEAVE_ROOM,
      this.boundHandlers.leaveRoom,
  ...
  _connect(roomId) {
    // NEXT_PUBLIC_ so the Next island can read the PartyKit host.
    const envHost =
      (typeof process !== "undefined" &&
        process.env?.NEXT_PUBLIC_MULTIPLAYER_SERVER_URL) ||
      null;
    const host = envHost ?? Constants.MULTIPLAYER.SERVER_URL;
    if (!host) {
      console.warn(
        "[NetworkManager] no SERVER_URL configured — multiplayer disabled",
      );
      this.gameState.multiplayer.connected = false;
  ...
  _onSocketClosed() {
    ...
    this.gameState.multiplayer.connected = false;
    this.registry.clear();
    this.eventBus.emit(Events.NETWORK_DISCONNECTED, {
      reason: this.intentionalDisconnect ? "closed" : "error",
    });
  ...
  _scheduleReconnect() {
    ...
    if (
      this.reconnectAttempts >= Constants.MULTIPLAYER.RECONNECT_MAX_ATTEMPTS
    ) {
    ...
    this.reconnectTimer = setTimeout(() => {
      this.reconnectTimer = null;
      this._connect(
        this.gameState.multiplayer.roomId ?? Constants.MULTIPLAYER.DEFAULT_ROOM,
      );
    }, delay);
  }
  ...
  _startPrune() {
    ...
    this.pruneInterval = setInterval(() => {
      ...
      for (const id of pruned)
        this.eventBus.emit(Events.NETWORK_PLAYER_LEFT, { playerId: id });
    }, intervalMs);
  }
...
Append to EventBus inside the `Events` constant, just before the closing `}`:
...
In `reset()`, append (with a `this.multiplayer` guard — `reset()` is often called from the GameState constructor, possibly _before_ the `this.multiplayer = {...}` block runs):
...
networkManager
  .init()
  .catch((err) => console.warn("[createGame] NetworkManager init failed", err));
```
...
networkManager
  .init()
  .catch((err) => console.warn("[createGame] NetworkManager init failed", err));
// NetworkManager is wired; GameState.multiplayer is the inspectable snapshot.
```

Patch 3 — GameState already holds `multiplayer` + `remotePlayers`. No `window` snapshot helper. Keep those fields in `reset()` (transient only).
...
In the active GameScene (the active GameScene):
...
In the orchestrator (`createGame` / Game.ts):
...
```
NEXT_PUBLIC_MULTIPLAYER_SERVER_URL=https://<project>.<user>.partykit.dev
```

Create `.env.example` (committed):

```
NEXT_PUBLIC_MULTIPLAYER_SERVER_URL=https://your-project.your-username.partykit.dev
```

The `NEXT_PUBLIC_` prefix is required so the Next island can read the PartyKit host.
