...
description: Add real-time or turn-based multiplayer to an existing browser game using PartyKit (Cloudflare Durable Objects). Scaffolds a room-based server, a NetworkManager client, EventBus events, GameState fields, and Constants. Use when the user says "add multiplayer", "make this multiplayer", "add real-time co-op", "add online play", "scaffold multiplayer", or "add netcode". Do NOT use for single-player AI opponents (use add-feature) or peer-to-peer/WebRTC games.
...
metadata:
  ...
  version: 0.1.0
  tags:
    [game, multiplayer, networking, partykit, websocket, realtime, turn-based]
compatibility: Requires an existing Phaser 3 or Three.js game with game-creator architecture (EventBus, GameState, Constants) under `app/<route>/game/`. Requires a Cloudflare account for `npx partykit deploy` (free tier is sufficient).
---
...
2. A **client `NetworkManager`** wired through EventBus (same seam as audio — no direct scene imports).
3. **Additive edits** to `EventBus`, `GameState`, and `Constants` — single-player gameplay must remain identical when the server is unreachable.
...
- `client-integration.md` — `MultiplayerClient`, `NetworkManager`, `RemotePlayerRegistry` source, EventBus/GameState/Constants append patterns.
- `partykit-server.md` — `npx partykit dev` and `npx partykit deploy`, capturing the deployed URL, `.env` handling.
...
2. **Additive edits only.** Append to EventBus, GameState, Constants, and `createGame` under a `// === Multiplayer ===` banner. Never rename, remove, or change existing fields.
...
- An existing Phaser 3 or Three.js game island under `app/<route>/game/` (EventBus, GameState, Constants, `createGame`).
...
Parse `$ARGUMENTS` for the game path and `--mode` flag. If no path, use `app/play/`. Verify Phaser or Three.js is in the app `package.json`.

Read these files in full before touching anything:

- App `package.json` — engine deps.
- `app/<route>/game/` — `createGame`, EventBus, GameState, Constants.
...
- `app/<route>/game/multiplayer/MultiplayerClient.ts` — backend-agnostic interface around `partysocket` (`connect`, `send`, `onMessage`, `disconnect`, `isConnected`).
- `app/<route>/game/multiplayer/RemotePlayerRegistry.ts` — `Map<playerId, RemotePlayer>` with `upsert`, `remove`, `prune(staleMs)`, `list()`.
- `app/<route>/game/systems/NetworkManager.ts` — wires MultiplayerClient ↔ EventBus, owns the broadcast tick (in `realtime` mode), handles reconnect with exponential backoff, emits `network:*` events.
...
**EventBus** — append under `// === Multiplayer ===` banner:
...
**GameState** — append a `multiplayer` field with persistent (`roomId`, `playerId`) and transient (`connected`, `remotePlayers`) parts. Update `reset()` to clear only the transient parts so rejoin works after a game restart.

**Constants** — append a `MULTIPLAYER` block with `SERVER_URL` (filled by Step 6), `DEFAULT_ROOM`, `MAX_PLAYERS`, `TICK_RATE_HZ`, reconnect backoff, stale-player threshold, `PROTOCOL_VERSION`. No magic numbers — every value is a named constant.

**`createGame`** — instantiate NetworkManager after EventBus + GameState, before the engine starts.
...
This starts a local CF Worker emulator on `http://127.0.0.1:1999`. Set `NEXT_PUBLIC_MULTIPLAYER_SERVER_URL=http://127.0.0.1:1999` in the app `.env` and preview this Next app.
...
This uses GitHub's device-code OAuth flow. The CLI prints a code; the user visits `https://github.com/login/device`, pastes it, and authorizes. Credentials persist in `~/.partykit/config.json`.
...
1. Game Constants → `MULTIPLAYER.SERVER_URL`
2. `.env` → `NEXT_PUBLIC_MULTIPLAYER_SERVER_URL=https://...`
3. `.env.example` → same key, placeholder URL

Add `.env` to `.gitignore` if not already present.

### Step 7: Preview the client

Preview this Next app. **PageScreenshot** `/play`.

### Step 8: Verify

**Single-player fallback (critical):** with the partykit dev server stopped, **PageScreenshot** `/play`. The game must boot, play, and reset normally. Confirm `network:disconnected` fired. **If the game depends on the server to start, you violated Principle 1 — revise.**

**Two-tab smoke test:** start `npx partykit dev`, preview this Next app, open two tabs on `/play`. Confirm:

- Both tabs fire `network:connected`.
- Each tab's GameState `multiplayer.remotePlayers` includes the other tab.
- Moving the local entity in tab A is reflected in tab B within `1000 / TICK_RATE_HZ * 2` ms.
...
**Regression:** boot, score, game-over, reset still hold whether the server is up or down. Walk [gameplay-invariants.md](../game-qa/gameplay-invariants.md).
...
1. **What was added** — server in `multiplayer-server/`, client under `app/<route>/game/multiplayer/` + `NetworkManager`, additive edits to EventBus / GameState / Constants / `createGame`.
2. **The server URL** — `https://<project>.<user>.partykit.dev`. Already wired into Constants and `.env`.
3. **How to test locally** — `cd multiplayer-server && npx partykit dev`, preview this Next app, open two tabs on `/play`.
4. **The single seam for backend swaps** — `MultiplayerClient.ts`. Future Colyseus or fly.io migration only changes that one file.
...
Result: detects engine, scaffolds `multiplayer-server/` with the realtime template, creates client networking files, deploys the PartyKit server, **PageScreenshot**s `/play`, prints the server URL.
...
**Fix:** Check `gameState.multiplayer.roomId` on both tabs — it should be the same (default `'lobby'`). If different, audit `NetworkManager.connect()` for stray query-string parsing. The server template's `room.broadcast(message, [sender.id])` excludes the sender, which is correct — each client renders only _remote_ players, not itself.
...
**Fix:** Audit `MultiplayerClient.connect()` and `NetworkManager.init()` — both must catch all errors, log a warning, emit `network:disconnected`, and return. The constructor and `init()` must never throw out of `createGame`.
...
**Fix:** Confirm `Constants.MULTIPLAYER.SERVER_URL` is the full `https://...partykit.dev` URL. `partysocket` derives the WSS URL by replacing the scheme. The deployed game must be served over HTTPS.
...
>
> For graduation to a more featureful backend (matchmaking, schema sync, server-authoritative physics), the only file that needs to change is `app/<route>/game/multiplayer/MultiplayerClient.ts`. Replace the `partysocket` calls with Colyseus's `colyseus.js` client; keep the same public API (`connect`, `send`, `onMessage`, `disconnect`, `isConnected`).
>
> The server runs in _your_ Cloudflare account, not OpusGameLabs'. Costs and quotas accrue to you. PartyKit itself is open source and free; you only pay Cloudflare's pass-through pricing (free tier is generous).
