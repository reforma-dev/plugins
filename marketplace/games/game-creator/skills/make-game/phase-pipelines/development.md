...
**3. Write the check first**

Before changing implementation code, name how you will prove the milestone's acceptance criteria. If a piece of AC has no check, it isn't really checkable. The work for this step is "done" when:

- Every checkable AC has a screenshot, a GameState read, or an explicit "user playtest" note.
- You know what fail looks like (blank canvas, score stuck, restart dirty) before you implement.

How to check:

- **Gameplay logic, state transitions, scoring, collision, win/lose** → **PageScreenshot** `/play` and walk [gameplay-invariants.md](../../game-qa/gameplay-invariants.md). Read GameState in source for fields the change should have touched.
- **Pure functions** (math helpers, formatters) → unit tests next to the source if the project already has a runner. Don't introduce a new test framework just for one helper.
- **Visual-only changes** → **PageScreenshot** before and after. For polish that can't be asserted, fall back to live-iterate and **explicitly note in the milestone that the AC is verified by user playtest**.
- **Multiplayer** → two preview tabs; GameState `remotePlayers` converges. The `add-multiplayer` skill has the smoke steps.

If the milestone AC is too vague ("combat should feel snappy"), stop and run [playtest / repro](../sub-pipelines/playtest-repro.md) first.

**4. Implement the smallest correct change**

- Implement only behavior outlined by the relevant milestone — the goal is to turn the checks from step 3 into passes.
...
- **PageScreenshot** after meaningful edits. If an invariant fails, fix the game — never weaken the check.
...
| Screenshot pass (boot / score / fail / restart)  | `/qa-game`                                                |
...
If the change includes visual or logical behavior the user can experience, give them clear, ordered testing instructions — one workflow at a time. Pair this with the screenshot from live-iterate so they have something concrete to compare against. Use `AskUserQuestion` when the next decision is multiple choice.
...
6. If `docs/gameplan.md` _is_ satisfied (loop, rules, win/lose, player goals all delivered), surface the highest value × ease item from `docs/backlog.md` via `AskUserQuestion` and ask the user whether it should be promoted into a stretch milestone or whether the gameplan itself needs to grow.
