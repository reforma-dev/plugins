...
1. Create `app/play/game/audio/AudioManager.ts` — AudioContext + sequencer + master gain
2. Create `app/play/game/audio/music.ts` — BGM patterns as note arrays + sequencer calls
3. Create `app/play/game/audio/sfx.ts` — SFX using Web Audio API (oscillator + gain + filter)
4. Create `app/play/game/audio/AudioBridge.ts` — wire EventBus events to audio
5. Wire `initAudioBridge()` in `createGame`
