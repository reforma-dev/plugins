...
Author is `plugin.json` `author.name` when present, else Reforma. Category is the Reforma marketplace id. Remote pins stamp `category` on the marketplace row.

## Repository structure

Root `marketplace.json` lists plugins. `source` is a path in this repo or a GitHub `tree/…` URL.

Local plugins live under `marketplace/<category>/<name>/`. Origin (first-party vs vendored) is `plugin.json` `author`, not the folder tree. Remote pins stay as GitHub URLs. Shelf ids and section order: `marketplace.json` `categories[]`. Empty shelves have no folder yet.
